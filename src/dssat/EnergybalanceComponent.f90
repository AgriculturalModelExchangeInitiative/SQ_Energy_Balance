...
CONTAINS
    ...
    !                          ** unit : degC
    !                          ** uri : http://www1.clermont.inra.fr/siriusquality/?page_id=547
        call model_diffusionlimitedevaporation(deficitOnTopLayers, soilDiffusionConstant,diffusionLimitedEvaporation)
        call model_conductance(vonKarman, heightWeatherMeasurements, zm, zh, d, plantHeight, wind,conductance)
        call model_netradiation(minTair, maxTair, albedoCoefficient, stefanBoltzman, elevation, solarRadiation, vaporPressure, extraSolarRadiation,netRadiation,netOutGoingLongWaveRadiation)
        call model_netradiationequivalentevaporation(lambdaV, netRadiation,netRadiationEquivalentEvaporation)
        call model_priestlytaylor(netRadiationEquivalentEvaporation, hslope, psychrometricConstant, Alpha,evapoTranspirationPriestlyTaylor)
        call model_penman(evapoTranspirationPriestlyTaylor, hslope, VPDair, psychrometricConstant, Alpha, lambdaV, rhoDensityAir, specificHeatCapacityAir, conductance,evapoTranspirationPenman)
        call model_evapotranspiration(isWindVpDefined, evapoTranspirationPriestlyTaylor, evapoTranspirationPenman,evapoTranspiration)
        call model_potentialtranspiration(evapoTranspiration, tau,potentialTranspiration)
        call model_ptsoil(evapoTranspirationPriestlyTaylor, Alpha, tau, tauAlpha,energyLimitedEvaporation)
        call model_soilevaporation(diffusionLimitedEvaporation, energyLimitedEvaporation,soilEvaporation)
        call model_soilheatflux(netRadiationEquivalentEvaporation, tau, soilEvaporation,soilHeatFlux)
        call model_cropheatflux(netRadiationEquivalentEvaporation, soilHeatFlux, potentialTranspiration,cropHeatFlux)
        call model_canopytemperature(minTair, maxTair, cropHeatFlux, conductance, lambdaV, rhoDensityAir, specificHeatCapacityAir,minCanopyTemperature,maxCanopyTemperature)
